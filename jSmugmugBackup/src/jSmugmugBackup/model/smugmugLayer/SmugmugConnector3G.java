...
public class SmugmugConnector3G implements ISmugmugConnector
{
    /* connecting to Smugmug through API version 1.2.0 */
	...
	public Number login(String userEmail, String password)
	{
        ...
        if (this.config.getRtconfigAnonymousLogin())
        {
            ...
            SmugmugConnector3G.login_nickname = userEmail;
            return 0;
        }
        else
        {
	...
	public void logout()
	{
		if (SmugmugConnector3G.login_sessionID != null) { this.smugmug_logout_logout(); }
		else
		{
			//just to be sure
        	SmugmugConnector3G.login_sessionID    = null;
        	SmugmugConnector3G.login_userID       = null;
        	SmugmugConnector3G.login_nickname     = null;
        	SmugmugConnector3G.login_passwordHash = null;
		}
	}
	public IRootElement getTree(String albumPassword)
	{
        ...
        SmugmugConnector3G.albumCache = null;
        if (this.config.getPersistentCacheAccountInfo())
        {
            if (this.config.getRtconfigAnonymousLogin() == false)
            {
                SmugmugConnector3G.albumCache = new SmugmugLocalAlbumCache(SmugmugConnector3G.login_userID.toString());
            }
            else
            {
		...
		while (jsonCategory != null)
		{
			...
			while (jsonSubcategory != null)
			{
				...
				while (jsonAlbum != null)
				{
                    ...
                    String albumKey      = (String)this.getJSONValue(jsonAlbum, "Key");
                    ...
                    if (this.config.getPersistentCacheAccountInfo())
                    {
                        if (SmugmugConnector3G.albumCache.exists(albumID.intValue()))
                        {
                            album = new Album(subcategory, SmugmugConnector3G.albumCache.getCachedAlbum(albumID.intValue()), this.getAlbumStatistics(albumID.intValue()));
                    ...
                    if (album == null)
                    {
                        album = new Album(subcategory, albumID.intValue(), albumName, albumKey, albumKeywords, albumLastUpdated, this.getAlbumStatistics(albumID.intValue()));
                        subcategory.addAlbum(album);

                        //iterate over images
                        JSONObject jsonImages = (JSONObject)this.smugmug_images_get(album.getID(), albumKey, albumPassword, null);
...
//                        }
			...
			while (jsonAlbum != null)
			{
                ...
                if (this.config.getPersistentCacheAccountInfo())
                {
                    if (SmugmugConnector3G.albumCache.exists(albumID.intValue()))
                    {
                        album = new Album(category, SmugmugConnector3G.albumCache.getCachedAlbum(albumID.intValue()), this.getAlbumStatistics(albumID.intValue()));
                ...
                if (album == null)
                {
                    album = new Album(category, albumID.intValue(), albumName, albumKey, albumKeywords, albumLastUpdated, this.getAlbumStatistics(albumID.intValue()) );
                    category.addAlbum(album);
...
//                    }
    ...
    {
        int imageIndex = 0;
        JSONObject jsonImage = (JSONObject)this.getJSONValue(jsonImages, "Album.Images[" + imageIndex + "]");
        //this.printJSONObject(jsonImage);
        while (jsonImage != null)
        {
            Long imageID              = (Long)this.getJSONValue(jsonImage, "id");
            String imageKey           = (String)this.getJSONValue(jsonImage, "Key");
            String imageName          = (String)this.getJSONValue(jsonImage, "FileName");
            String imageCaption       = (String)this.getJSONValue(jsonImage, "Caption");
            String imageKeywords      = (String)this.getJSONValue(jsonImage, "Keywords");
            String imageFormat        = (String)this.getJSONValue(jsonImage, "Format");
            Long imageHeight          = (Long)this.getJSONValue(jsonImage, "Height");
            Long imageWidth           = (Long)this.getJSONValue(jsonImage, "Width");
            Long imageSize            = (Long)this.getJSONValue(jsonImage, "Size");
            String imageMD5           = (String)this.getJSONValue(jsonImage, "MD5Sum");
            Double imageLatitude      = (Double)this.getJSONValue(jsonImage, "Latitude");
            Double imageLongitude     = (Double)this.getJSONValue(jsonImage, "Longitude");
            Long imageAltitude        = (Long)this.getJSONValue(jsonImage, "Altitude");
            String imageAlbumURL      = (String)this.getJSONValue(jsonImage, "AlbumURL");
            String imageThumbURL      = (String)this.getJSONValue(jsonImage, "ThumbURL");
            String imageTinyURL       = (String)this.getJSONValue(jsonImage, "TinyURL");
            String imageSmallURL      = (String)this.getJSONValue(jsonImage, "SmallURL");
            ...
            */
            ...
            IImage image = new Image(album, imageID.intValue(), imageName, imageKey, imageCaption, imageKeywords, imageFormat, imageHeight.intValue(), imageWidth.intValue(), imageSize.longValue(), /*largestURLContentSize,*/
                                     ...
                                     imageVideo320URL, imageVideo640URL, imageVideo960URL, imageVideo12800URL);
            album.addImage(image);
            ...
            imageIndex++;
            jsonImage = (JSONObject)this.getJSONValue(jsonImages, "Album.Images[" + imageIndex + "]");
        }
    }
	public IAlbum getAlbum(int albumID, String albumKey, String albumPassword)
	{
		...
		JSONObject jsonImages = this.smugmug_images_get(albumID, albumKey, albumPassword, null);
		this.printJSONObject(jsonImages);


        IAlbum album = new Album(null, albumID, "nokey", "Album Doe", "", "", new Vector<IAlbumMonthlyStatistics>());
        this.getTree_iterateImages(jsonImages, album, null);

        return album;
	}
	public Hashtable<String, String> getImageInfo(int imageID, String imageKey, String albumPassword)
	{
        if (imageID == 0) { return null; }

		Hashtable<String, String> result = new Hashtable<String, String>();

		JSONObject jobj = this.smugmug_images_getInfo(imageID, imageKey, albumPassword, null);
...
//	public void downloadFile(int imageID, String imageKey, String albumPassword, File fileName, /*long expectedFilesize,*/ /*ResolutionEnum minResolution,*/ ResolutionEnum maxResolution)
//	{
//    	//JSONObject jobj = this.smugmug_images_getURLs(imageID); //retrieves just the urls
//		JSONObject jobj = this.smugmug_images_getInfo(imageID, imageKey, albumPassword, null); // get image_info, including url
//		//this.printJSONObject(jobj);
//
//
...
//        if (maxResolution == null) { maxResolution = ResolutionEnum.Original; }
//
...
//        else if ((this.getJSONValue(jobj, "Image.OriginalURL") != null) && (maxResolution.compareTo(ResolutionEnum.Original) >= 0)) { imageURL = (String)this.getJSONValue(jobj, "Image.OriginalURL"); }
//        else if ((this.getJSONValue(jobj, "Image.X3LargeURL") != null) && (maxResolution.compareTo(ResolutionEnum.X3Large) >= 0))
//        {
//            imageURL = (String)this.getJSONValue(jobj, "Image.X3LargeURL");
//            fileNameStr = fileNameStr.substring(0, fileNameStr.lastIndexOf(".")-1) + "-X3" + fileNameStr.substring(fileNameStr.lastIndexOf("."));
//        }
//        else if ((this.getJSONValue(jobj, "Image.X2LargeURL") != null) && (maxResolution.compareTo(ResolutionEnum.X2Large) >= 0))
//        {
//            imageURL = (String)this.getJSONValue(jobj, "Image.X2LargeURL");
//        }
//        else if ((this.getJSONValue(jobj, "Image.XLargeURL") != null) && (maxResolution.compareTo(ResolutionEnum.XLarge) >= 0))
//        {
//            imageURL = (String)this.getJSONValue(jobj, "Image.XLargeURL");
//        }
//        else if ((this.getJSONValue(jobj, "Image.LargeURL") != null) && (maxResolution.compareTo(ResolutionEnum.Large) >= 0))
//        {
//            imageURL = (String)this.getJSONValue(jobj, "Image.LargeURL");
//        }
//        else if ((this.getJSONValue(jobj, "Image.MediumURL") != null) && (maxResolution.compareTo(ResolutionEnum.Medium) >= 0))
//        {
//            imageURL = (String)this.getJSONValue(jobj, "Image.MediumURL");
//        }
//        else if ((this.getJSONValue(jobj, "Image.SmallURL") != null) && (maxResolution.compareTo(ResolutionEnum.Small) >= 0))
//        {
//            imageURL = (String)this.getJSONValue(jobj, "Image.SmallURL");
//        }
//        //else if (this.getJSONValue(jobj, "Image.TinyURL") != null) { imageURL = (String)this.getJSONValue(jobj, "Image.TinyURL"); }
//        //else if (this.getJSONValue(jobj, "Image.ThumbURL") != null) { imageURL = (String)this.getJSONValue(jobj, "Image.ThumbURL"); }
//        else
//        {
//            this.printJSONObject(jobj);
...
//
//        File targetFileName = new File(fileNameStr);
//		//this.log.printLogLine("INFO: targetFileName = " + targetFileName);
//        this.downloadFile(imageURL, targetFileName/*, expectedFilesize*/);
//	}
...
//            } while (tempFileName.length() != contentFilesize);
            ...
            if (!renameResult) { this.log.printLogLine(LogLevelEnum.Error, 0, "ERROR: renaming temporary file to " + fileName + " failed - this might be a platform specific issue"); System.exit(1); }
	...
	private JSONObject smugmugJSONRequest(HttpRequestBase httpRequest)
	{
        ...
        if (jobj == null)
        {
        	this.log.printLogLine(LogLevelEnum.Error, 0, "ERROR: jobj == null ... this is unusual");
        	this.log.printLogLine(LogLevelEnum.Error, 0, responseBody);
        }
	...
	private void smugmug_login_anonymously(boolean beQuiet)
	{
		...
		do
		{
	        ...
	        if ( (this.getJSONValue(jobj, "stat").equals("ok")) &&
	        	 (this.getJSONValue(jobj, "method").equals(methodName)) )
	        {
	        	if (!beQuiet) { this.log.printLogLine(LogLevelEnum.Message, 0, "ok"); }
	        	SmugmugConnector3G.login_sessionID    = (String)this.getJSONValue(jobj, "Login.Session.id");
	        	return;
	        }
	        else
	...
	private void smugmug_logout_logout()
	{
        ...
        else { this.log.printLogLine(LogLevelEnum.Message, 0, "failed"); }
	}
	...
	{
        /* transfer stats are not yet implemented in API version 1.2.2, so
         * we're using 1.2.0 here
         */
		...
		//build url
		String url = this.config.getConstantSmugmugServerURL_120() + "?"; //using API version 1.2.0 here!!!
		url = url + "method=" + methodName + "&";
		url = url + "SessionID=" + SmugmugConnector3G.login_sessionID + "&";
		url = url + "Month=" + month + "&";
        url = url + "Year=" + year + "&";
	...
	private JSONObject smugmug_categories_create(String name)
	{
        ...
        {
        	this.log.printLogLine(LogLevelEnum.Message, 0, "creating category failed");
        	this.printJSONObject(jobj);
        }
	...
	private JSONObject smugmug_categories_rename(int categoryID, String newName)
	{
        ...
        if ( (this.getJSONValue(jobj, "stat").equals("ok")) &&
             (this.getJSONValue(jobj, "method").equals(methodName)) )
        {
        	this.log.printLogLine(LogLevelEnum.Message, 0, "ok (id=" + this.getJSONValue(jobj, "Category.id") + ")");
        	return jobj;
        }
        else
        {
    ...
    private JSONObject smugmug_subcategories_create(String name, int categoryID)
	{
        ...
        {
        	this.log.printLogLine(LogLevelEnum.Message, 0, "creating subcategory failed");
        	this.printJSONObject(jobj);
        }
	...
	private JSONObject smugmug_albums_create(String title, int categoryID, int subCategoryID, String albumKeywords)
	{
        ...
        {
        	this.log.printLogLine(LogLevelEnum.Message, 0, "creating album failed");
        	this.printJSONObject(jobj);
        }
	...
	private JSONObject smugmug_albums_changeSettings_position(int albumID, int newPosition)
	{
		...
		JSONObject jobj = this.smugmugJSONRequest(httpget);
		//this.printJSONObject(jobj);
		this.log.printLogLine(LogLevelEnum.Message, 0, "url: " + url);
	...
	private JSONObject smugmug_images_get(int albumID, String albumKey, String albumPassword, String sitePassword)
	{
		...
		url = url + "SessionID=" + SmugmugConnector3G.login_sessionID + "&";
		url = url + "AlbumID=" + albumID + "&";
        url = url + "AlbumKey=" + albumKey + "&";
		url = url + "Heavy=1&"; //optional
		if (albumPassword != null) { url = url + "Password=" + albumPassword + "&"; } //optional
		if (sitePassword != null)  { url = url + "SitePassword=" + sitePassword + "&"; } //optional
...
//	        }
	        ...
	        {
                //invalid user ... probably a missing site/album password in conjunction with anonymous login ... ignoring this
                //this.log.printLogLine("WARNING: " + this.getJSONValue(jobj, "message") + " ... albumID=" + albumID + " albumKey=" + albumKey);
	        	//this.printJSONObject(jobj);
                return jobj;
	        }
	        else
	...
	private JSONObject smugmug_images_changeSettings(int imageID, String keywords)
	{
		...
		do
		{
	        ...
	        {
	        	this.log.printLogLine(LogLevelEnum.Message, 0, "nothing changed");
                //this.log.printLog("_");
	        	return jobj;
	        }
	        else
	...
	private JSONObject smugmug_images_getURLs(int imageID)
	{
        ...
        {
        	this.log.printLogLine(LogLevelEnum.Message, 0, "failed");
        	this.log.printLogLine(LogLevelEnum.Message, 0, Helper.getCurrentTimeString() + " " + methodName + " ... failed");
        }

        return null;
    }
	private JSONObject smugmug_images_getInfo(int imageID, String imageKey, String albumPassword, String sitePassword)
	{
		...
		url = url + "SessionID=" + SmugmugConnector3G.login_sessionID + "&";
		url = url + "ImageID=" + imageID + "&"; //integer
		url = url + "ImageKey=" + imageKey + "&"; //string
		if (albumPassword != null) { url = url + "Password=" + albumPassword + "&"; } //string, optional
		if (sitePassword != null)  { url = url + "SitePassword=" + sitePassword + "&"; } //string, optional
	...
	private JSONObject smugmug_images_upload(int albumID, File fileName, String caption, String keywords)
	{
		...
		do
		{
            ...
            try //temporary
            {
                ...
                {
                    this.log.printLogLine(LogLevelEnum.Message, 0, "failed (wrong format)");
                    this.log.printLogLine(LogLevelEnum.Error, 0, "  ERROR: the file format was not recognized by SmugMug");
                    this.log.printLogLine(LogLevelEnum.Error, 0, "  ERROR: maybe it's neither a picture, nor a video ... or the video is too long?");
                    this.log.printLogLine(LogLevelEnum.Error, 0, "  ERROR: see: http://www.smugmug.com/homepage/uploadlog.mg");
            ...
            {
                this.log.printLogLine(LogLevelEnum.Exception, 0, "caught NullPointer exception: " + ne.getMessage());
                ne.printStackTrace();
                this.printJSONObject(jobj);
                System.exit(1);
            }
	...
	{
		this.log.printLogLine(LogLevelEnum.Debug, 0, "DEBUG: printing JSONObject ...");
		this.log.printLogLine(LogLevelEnum.Debug, 0, "DEBUG: jobj=" + jobj);
	...
	private void printJSONObject(JSONObject jobj, String indent)
	{
		for (int i = 0; i < jobj.keySet().size(); i++)
		{
			...
			{
				this.log.printLogLine(LogLevelEnum.Debug, 0, "DEBUG:" + indent + key + ": ");
				this.printJSONObject((JSONObject)value, indent + "   ");
			}
			else if (value instanceof JSONArray)
			{
				this.log.printLogLine(LogLevelEnum.Debug, 0, "DEBUG:" + indent + key + " (Array)");
			...
			else this.log.printLogLine(LogLevelEnum.Debug, 0, "DEBUG:" + indent + key + ": " + value);
    ...
    private synchronized void cacheCleanup(int albumID)
    {
        if (this.config.getPersistentCacheAccountInfo())
        {
            ...
            */
            ...
            if (SmugmugConnector3G.albumCache == null) { this.log.printLogLine(LogLevelEnum.Debug, 0, "ERROR: album cache has not been initialized"); System.exit(1); }
